...
const ChangeInCasesOverTime = ({
  ...
  useEffect(() => {
    async function getInfo() {
      ...
      console.log(
        new Date(startYear + 1, startMonth - 1, startDay).toISOString()
      );
      const start = await getDisease(
        startDate,
        new Date(startYear + 1, startMonth - 1, startDay).toISOString(),
        disease,
        country
      );
      const yr1 = await getDisease(
        startDate,
        new Date(endYear - 5, endMonth - 1, endDay).toISOString(),
        disease,
        country
      );
      const yr2 = await getDisease(
        startDate,
        new Date(endYear - 4, endMonth - 1, endDay).toISOString(),
        disease,
        country
      );
      const yr3 = await getDisease(
        startDate,
        new Date(endYear - 3, endMonth - 1, endDay).toISOString(),
        disease,
        country
      );
      const yr4 = await getDisease(
        startDate,
        new Date(endYear - 2, endMonth - 1, endDay).toISOString(),
        disease,
        country
      );
      const yr5 = await getDisease(
        startDate,
        new Date(endYear - 1, endMonth - 1, endDay).toISOString(),
        disease,
        country
      );
      const yr6 = await getDisease(startDate, endDate, disease, location);
      const currCases = await getDisease(
        startDate,
